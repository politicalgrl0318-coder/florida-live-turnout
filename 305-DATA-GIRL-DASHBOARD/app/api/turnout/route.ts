...
export async function GET(){const rows=await Promise.all(Object.entries(counties).map(([c,n])=>c==="BRO"?browardCounty().catch(()=>county(c,n)):county(c,n)));const first=rows.find(r=>r.status==="live");return NextResponse.json({generatedAt:new Date().toISOString(),electionName:first?.electionName||"2026 Primary Election",electionDate:first?.electionDate||"08/18/2026",counties:rows},{headers:{"Cache-Control":"no-store, no-cache, must-revalidate, max-age=0","CDN-Cache-Control":"no-store","Cloudflare-CDN-Cache-Control":"no-store","Pragma":"no-cache","Expires":"0"}})}
