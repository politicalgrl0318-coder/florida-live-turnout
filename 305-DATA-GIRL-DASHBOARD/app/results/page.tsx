...
type CountyResult={name:string;url:string;isFallback:boolean};
...
const countyResults:CountyResult[]=names.map(name=>({
  ...
  isFallback:!augustPrimaryLinks[name],
}));
export default function ResultsPage(){const[query,setQuery]=useState("");const counties=useMemo(()=>countyResults.filter(c=>c.name.toLowerCase().includes(query.trim().toLowerCase())),[query]);const direct=countyResults.filter(c=>!c.isFallback).length;return <main className="results-page"><header className="results-hero"><nav aria-label="Main navigation"><a href="/">Turnout dashboard</a><a href="/landscape">County landscape</a><a className="active" href="/results" aria-current="page">Official results</a></nav><div className="results-brand"><span>305</span> Data Girl</div><p className="results-kicker">FLORIDA ELECTION DIRECTORY</p><h1>Florida August 2026 Primary Election Results</h1><p className="results-intro">Go straight to the government source for vote totals and race outcomes across Florida’s 67 counties.</p></header><section className="results-content"><article className="statewide-card"><div><span className="official-pill">OFFICIAL STATEWIDE SOURCE</span><h2>Federal, statewide &amp; multicounty races</h2><p>Florida Election Watch publishes results reported to the Florida Department of State. Use it for contests that cross county lines.</p></div><a href={electionWatchUrl} target="_blank" rel="noreferrer">Open Florida Election Watch <span>↗</span></a></article><aside className="results-explainer">Results are unofficial until canvassing and certification are complete</aside><div className="county-directory-head"><div><p className="results-kicker">COUNTY-BY-COUNTY</p><h2>Find your county’s results</h2><span>{direct} direct county pages · {67-direct} Florida Election Watch fallbacks</span></div><label><span>Search counties</span><input aria-label="Search counties" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Start typing a county…"/></label></div><div className="county-result-grid">{counties.map(c=><article key={c.name}><div><span className="county-mark">{c.name.slice(0,2).toUpperCase()}</span><div><h3>{c.name} County</h3><p>{c.isFallback?"Official state reporting":"Supervisor of Elections"}</p></div></div><a href={c.url} target="_blank" rel="noreferrer" aria-label={`Open official ${c.name} County election results`}>{c.isFallback?"Florida Election Watch":"Official results"} <span>↗</span></a></article>)}</div>{!counties.length&&<p className="no-counties">No counties match “{query}.”</p>}<p className="directory-note"><b>Link note:</b> County election websites can change their election-specific addresses. Counties without a stable direct results-page link use Florida Election Watch, the official state reporting source. Links open official government election resources in a new tab.</p></section></main>}
