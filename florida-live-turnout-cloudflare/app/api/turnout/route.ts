...
async function browardCounty(){
  ...
  const mail=demTypes[0]+repTypes[0]+npaTypes[0]+otherTypes[0];
  const early=demTypes[1]+repTypes[1]+npaTypes[1]+otherTypes[1];
  const electionDay=demTypes[2]+repTypes[2]+npaTypes[2]+otherTypes[2];
  ...
  return{code:"BRO",name:"Broward",sourceUrl,status:"live",registered,ballots,turnout:registered?ballots/registered*100:0,mail,early,electionDay,dem,rep,npa,other,updated,electionName:"2026 Primary Election",electionDate:"08/18/2026"};
}
async function county(code:string,name:string){try{const ir=await fetch(`${base}/${code}/index.json?${Date.now()}`,{cache:"no-store"});if(!ir.ok)throw new Error();const elections=await ir.json() as(string|number)[];const election=[...elections].sort()[0];const r=await fetch(`${base}/${code}/${election}/data.json?${Date.now()}`,{cache:"no-store"});if(!r.ok)throw new Error();const j=await r.json() as any,p=j.Turnout?.PartyType||{},days=Object.values(j.Turnout?.DateType||{})as Record<string,number>[];const mail=days.reduce((a,d)=>a+(d.Mail||0),0),early=days.reduce((a,d)=>a+(d.EarlyVoting||0),0),electionDay=days.reduce((a,d)=>a+(d.ElectionDay||0),0),dem=sum(p.DEM),rep=sum(p.REP),npa=sum(p.NPA);const all=Object.values(p).reduce((a,v)=>a+sum(v as Record<string,number>),0),ballots=mail+early+electionDay,registered=Number(j.Summary?.ActiveEligibleVoters||j.Summary?.TotalRegisteredVoters||0);return{code,name,sourceUrl:source(code),status:"live",registered,ballots,turnout:registered?ballots/registered*100:0,mail,early,electionDay,dem,rep,npa,other:Math.max(0,all-dem-rep-npa),updated:j.Summary?.LastUpdatedTime||null,electionName:j.Summary?.ElectionName||"",electionDate:j.Summary?.ElectionDate||""}}catch{return{code,name,sourceUrl:source(code),status:"unavailable",registered:0,ballots:0,turnout:0,mail:0,early:0,electionDay:0,dem:0,rep:0,npa:0,other:0,updated:null,electionName:"",electionDate:""}}}
export async function GET(request:Request){const requested=new URL(request.url).searchParams.get("codes")?.split(",").filter(code=>counties[code]);const entries=requested?.length?requested.map(code=>[code,counties[code]] as const):Object.entries(counties).slice(0,15);const rows=await Promise.all(entries.map(([c,n])=>c==="BRO"?browardCounty().catch(()=>county(c,n)):county(c,n)));const first=rows.find(r=>r.status==="live");return NextResponse.json({generatedAt:new Date().toISOString(),electionName:first?.electionName||"2026 Primary Election",electionDate:first?.electionDate||"08/18/2026",counties:rows},{headers:{"Cache-Control":"public, max-age=60, s-maxage=180, stale-while-revalidate=300"}})}
